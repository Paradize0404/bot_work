...
STORE_TYPE_MAP = {
    ...
    "хозы": {
        ...
        "name": "Хоз. товары (Московский)",
    },
}

BASE_MAPPING = {
    "молоко": {
        "iiko_id": "11111111-0000-0000-0000-000000000001",
        "iiko_name": "Молоко 1л",
        "store_type": "кухня",
        "type": "товар",
    },
    "масло": {
        "iiko_id": "22222222-0000-0000-0000-000000000001",
        "iiko_name": "Масло 82%",
        "store_type": "кухня",
        "type": "товар",
    },
    "пиво": {
        "iiko_id": "33333333-0000-0000-0000-000000000001",
        "iiko_name": "Пиво разл.",
        "store_type": "бар",
        "type": "товар",
    },
    "поставщик ооо": {
        "iiko_id": "99999999-0000-0000-0000-000000000001",
        "iiko_name": "ООО Поставщик",
        "store_type": "",
        "type": "поставщик",
    },
}
...
async def test_single_invoice_single_store(
    ...
    docs = [_doc()]
    invoices, warnings = await build_iiko_invoices(
...
async def test_reenrich_item_from_mapping(
    ...
    assert len(invoices) == 1
    assert (
        invoices[0]["items"][0]["productId"] == "11111111-0000-0000-0000-000000000001"
    )
...
async def test_empty_docs_returns_empty(
    ...
    invoices, warnings = await build_iiko_invoices(
        [], DEPT_ID, base_mapping=BASE_MAPPING
    )
    assert invoices == []
    assert warnings == []
