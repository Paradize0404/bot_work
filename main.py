...
def _build_bot_and_dp() -> tuple[Bot, Dispatcher]:
    ...
    # from bot.ocr_handlers import router as ocr_router  # Временно отключено для отладки промта
    ...
    # dp.include_router(ocr_router)  # Временно отключено для отладки промта
