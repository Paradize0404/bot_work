...
### 2026-03-16 — [FEAT] JSON-файл: отправитель получает только подтверждение, всё остальное — бухгалтеру

Пересмотрен поток обработки JSON-файлов (кассовых чеков).

**Было:** отправитель видел превью, кнопки «📤 Отправить в iiko» / «❌ Отменить», детали маппинга.
**Стало:** отправитель получает только «✅ Файл отправлен бухгалтеру на проверку». Превью, кнопки и уведомления о маппинге — **только бухгалтеру** (📑).

**Изменения:**
- `bot/document_handlers.py`:
  - `handle_json_receipt()`: при незамапленных — уведомление бухгалтерам (не sender), sender получает только подтверждение.
  - `_build_and_send_json_invoices()`: превью + кнопки отправляются только бухгалтерам, sender — подтверждение. Убран неиспользуемый параметр `chat_id`.
  - `cb_iiko_invoice_send()`: нажать может только бухгалтер (убрано разрешение для sender).
  - `cb_iiko_invoice_cancel()`: аналогично — только бухгалтер.

---

### 2026-03-16 — [FEAT] Отключена автоматическая отправка остатков при закрытии заказа

Остатки складов больше не отправляются автоматически по вебхуку (DeliveryOrderUpdate / TableOrderUpdate).
Отправка остатков теперь **только по кнопке** «🔄 Обновить остатки сейчас» (`force_stock_check`).

**Изменения:**
- `use_cases/iiko_webhook_handler.py`: из `handle_webhook()` убрана вся логика sync + check + send остатков при закрытых заказах. Закрытые заказы теперь только логируются.
- `force_stock_check()` — без изменений, работает как прежде (кнопка).
- `docs/WEBHOOKS.md`: обновлена архитектура и секция «Остатки: логика обновления».

**Причина:** снижение спама уведомлений — остатки актуальны только при явном запросе.
...
#### WRITEOFF-логика (главное изменение)
- Списания (TransactionType=WRITEOFF) записываются в «Списания продуктов».
- **Новое:** сумма WRITEOFF одновременно вычитается из той FT-категории, куда строка попала бы по маппингу (группа > счёт).
  - Пример: WRITEOFF `Кухня (Клин.)` → пишем в «Списания продуктов» и вычитаем из «Сырьевая себестоимость». Это предотвращает задвоение.
- Если WRITEOFF-строка не имеет маппинга исходной категории — логируем WARNING (вычет невозможен).
- Если после вычета категория уходит в 0 или минус — запись в FT не создаётся.

#### Контроль целостности
- В лог пишется: `iiko_total`, `allocated (FT)`, `unmapped`, `writeoff_deducted`.
- Проверка: `iiko = FT + unmapped + deducted`. При расхождении > 1₽ — WARNING.
- `unmapped_sums`: теперь трекаются **суммы** потерянных транзакций, а не только имена.
- Telegram-отчёт отображает: iiko итого → FT / списания вычтено / не разнесено.

#### Производительность
- **Батч GSheets:** новая функция `read_fintab_all_mappings()` — один запрос вместо двух (ОПИУ + подразделения). Экономия ~5-8 сек.
- **Параллельные FT API:** `asyncio.gather` для `_sync_one_category()` вместо последовательных вызовов.

#### Маппинг подразделений → направлений
- Улучшен `_resolve_department_direction()`: 3-фазный поиск (точное → нормализованное → подстрочное).
- `_normalize_dept()`: убирает «PizzaYolo», «Пицца Йоло», скобки, слэши — чтобы маппинг «Клиническая» совпал с OLAP «Клиническая PizzaYolo».

#### Файлы
| Файл | Что изменено |
|------|-------------|
| `use_cases/pnl_sync.py` | WRITEOFF-вычет, unmapped_sums, integrity check, parallel gather, batch GSheets, _normalize_dept |
| `adapters/google_sheets.py` | Новая `read_fintab_all_mappings()` |
| `bot/pnl_handlers.py` | `format_opiu_result` — суммы, writeoff_deducted в отчёте |
