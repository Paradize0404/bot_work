...
iikoCloud POST /iiko-webhook
  ...
  └─ DeliveryOrderUpdate / TableOrderUpdate (status=Closed)
      ...
      → покомпонентное сравнение: каждая позиция ниже минимума
      → триггер отправки если хотя бы одно из:
          • Любая позиция ≥ 3% изменение (STOCK_CHANGE_THRESHOLD_PCT)
          • Новые позиции ниже минимума
          • Позиции исчезли (стали выше минимума)
          • Прошло ≥ 30 мин с последней отправки И есть изменения
      → delete old msg → send new → pin
      └─ Получатели: роль «📦 Остатки» (fallback → все авторизованные)
```
...
**Триггеры отправки** (хотя бы один):
...
5. ✅ Прошло ≥ `STOCK_UPDATE_INTERVAL_MIN` (30 мин) + есть изменения
...
| Роль | Столбец GSheet | Получает |
|------|----------------|----------|
| 👑 Админ | «👑 Админ» | Bypass всех прав, отчёты sync |
