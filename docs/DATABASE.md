...
**Всего таблиц:** 42 (28 iiko/bot + 13 FinTablo + 1 внешняя)

---

## Таблицы iiko / бота (28)
...
### 39. `ocr_document` — OCR-распознанные бухгалтерские документы

Источник: Gemini Vision OCR (фото документа → JSON)

| Колонка | Тип | Описание |
|-------------|---------------|-------------------------------------------------------|
| `id` | Integer PK | Автоинкремент |
| `telegram_id` | BigInteger | Telegram ID пользователя (index) |
| `doc_type` | String(50) | Тип документа (УПД, Чек, РКО, Акт...) |
| `doc_number` | String(100) | Номер документа |
| `doc_date` | String(20) | Дата (эДД.ММ.ГГГГ» как распознал LLM) |
| `supplier_name` | String(500) | Название поставщика |
| `supplier_inn` | String(20) | ИНН поставщика |
| `buyer_name` | String(500) | Название покупателя |
| `items_count` | Integer | Количество позиций |
| `total_with_vat` | Numeric(14,2) | Итого с НДС |
| `category` | String(20) | `goods` = товар (→ iiko), `service` = услуга |
| `status` | String(30) | recognized → mapping → pending_approval → sent_to_iiko / acknowledged / rejected / error (index) |
| `raw_json` | JSONB | Полный JSON от LLM |
| `mapped_json` | JSONB | JSON после маппинга (с _product_id, _supplier_id) |
| `iiko_response` | Text | Ответ iiko при отправке |
| `created_at` | DateTime | Дата создания |
| `updated_at` | DateTime | Дата обновления |

**Индексы:** `ix_ocr_document_tg` (telegram_id), `ix_ocr_document_status` (status)
**Использование:** хранение всех OCR-документов с полным lifecycle (распознание → маппинг → иико / ознакомление).

---

### 40. `ocr_item_mapping` — Обучаемый маппинг товаров (OCR → iiko)

Источник: OCR (fuzzy match + ручной маппинг через GSheet)

| Колонка | Тип | Описание |
|----------------|---------------|-------------------------------------------------------|
| `id` | Integer PK | Автоинкремент |
| `raw_name` | String(500) | Название как распознал LLM (index, unique) |
| `corrected_name`| String(500) | Нормализованное название |
| `product_id` | UUID | UUID товара в iiko (→ iiko_product.id) |
| `product_name` | String(500) | Название товара из iiko |
| `created_at` | DateTime | Дата создания |

**Unique constraint:** `uq_ocr_item_mapping_raw_name` (raw_name)
**Индексы:** `ix_ocr_item_mapping_raw` (raw_name)
**Использование:** обучаемый маппинг — каждый ручной/авто маппинг сохраняется и используется при следующих распознаваниях. Через 50–100 документов система распознаёт почти всё автоматически.

---

### 41. `ocr_supplier_mapping` — Маппинг поставщиков (OCR → iiko + категория)

Источник: OCR (fuzzy match + обучение через кнопку «📋 Это услуга»)

| Колонка | Тип | Описание |
|----------------|---------------|-------------------------------------------------------|
| `id` | Integer PK | Автоинкремент |
| `raw_name` | String(500) | Название поставщика как распознал LLM (index, unique) |
| `raw_inn` | String(20) | ИНН поставщика |
| `supplier_id` | UUID | UUID поставщика в iiko (→ iiko_supplier.id) |
| `supplier_name` | String(500) | Название поставщика из iiko |
| `category` | String(20) | `goods` = товар (→ iiko), `service` = услуга |
| `created_at` | DateTime | Дата создания |

**Unique constraint:** `uq_ocr_supplier_mapping_raw_name` (raw_name)
**Индексы:** `ix_ocr_supplier_mapping_raw` (raw_name)
**Использование:** маппинг поставщиков + категория (товар/услуга). Бухгалтер нажимает «📋 Это услуга» — поставщик сохраняется с `category="service"`, при следующем документе от этого поставщика — автоопределение.
