...
### 14c. `writeoff_excluded_prepared_group` — Исключённые папки заготовок (обычные точки)

Корневые группы номенклатуры, исключённые из поиска PREPARED-товаров при списании на обычных точках.
BFS-обход потомков → PREPARED из этих папок **не отображаются** в поиске.
Если таблица пуста → все PREPARED показываются (без ограничений).
...
> Инициализация: INSERT INTO writeoff_excluded_prepared_group (group_id, group_name) VALUES ('uuid-здесь', 'Неиспользуемые заготовки');

---

### 14d. `writeoff_excluded_prepared_request_group` — Исключённые папки заготовок (точка-получатель)

Аналог `writeoff_excluded_prepared_group`, но для точки-получателя заявок (request store).
BFS-обход потомков → PREPARED из этих папок **не отображаются** в поиске.
...
> Инициализация: INSERT INTO writeoff_excluded_prepared_request_group (group_id, group_name) VALUES ('uuid-здесь', 'Неиспользуемые заготовки ЦК');
...
### 27b. `pnl_account_mapping` — Маппинг iiko Account → FinTablo PnL

Источник: бот (маппинг из Google Sheets «Маппинг FinTablo»)
ORM: `PnlAccountMapping` в `db/ft_models.py`

| Колонка | Тип | Описание |
|---------|-----|----------|
| `id` | Integer PK | Автоинкремент |
| `iiko_account_name` | String(500) | Имя счёта iiko (Account.Name из OLAP) (index) |
| `ft_pnl_category_id` | BigInteger | ID статьи ПиУ в FinTablo (index) |
| `ft_pnl_category_name` | String(500) | Имя статьи (кэш для UI) |
