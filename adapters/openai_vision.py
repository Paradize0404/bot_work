...
⚠️ ПРОВЕРКА КАЧЕСТВА ФОТО (СТРОГО ПО КРИТЕРИЯМ):
- Оцени качество фото и свою уверенность в правильности распознавания (0-100%)
- needs_retake = true ТОЛЬКО если невозможно прочитать критичные данные

**ЗОЛОТОЕ ПРАВИЛО**: Если ты смог распознать ВСЕ основные данные (поставщик, дата, номер, позиции с количеством и ценами, итоги) → needs_retake=false И issues=[] (пустой массив)

**Критические проблемы** (needs_retake=true):
  * Блики ПОЛНОСТЬЮ закрывают номер документа, дату, суммы или ИНН так что НЕВОЗМОЖНО прочитать
  * Размыто настолько что НЕВОЗМОЖНО разобрать цифры (уверенность распознавания чисел < 50%)
  * Отсутствует критичная часть документа (нет таблицы товаров, обрезана половина)

**НЕ критично** (needs_retake=false, НЕ упоминай в issues):
  * ✅ Наклон камеры любой степени — если данные читаются
  * ✅ Лёгкая/средняя размытость — если цифры различимы
  * ✅ Частичные блики на краях/фоне — если не закрывают важные данные
  * ✅ Уверенность 50-100% — достаточно для работы

**issues** — массив проблем:
  * Оставь ПУСТЫМ [] если распознал документ успешно (даже если наклон/блики/размытость)
  * Укажи проблему ТОЛЬКО если she МЕШАЕТ распознаванию критичных данных
  * Примеры: ["блик закрывает номер документа"], ["невозможно прочитать суммы"]
  * НЕ пиши: "наклон фото", "размытость", "слабое освещение" — если данные прочитаны
...
{{
  ...
  "items": [
    {{
      ...
      "unit": "шт/кг/л/уп",
      "packaging": "фасовка (например, '10 шт в уп' или null если не указано)",
      "qty": 1.0,
      "price": 100.00,
