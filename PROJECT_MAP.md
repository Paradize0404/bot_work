...
| `update_product(product_id, fields)` | `/resto/api/v2/entities/products/update` | JSON | product_id + dict полей (напр. storeBalanceLevels) |
...
│   │                         #   Подменю «Отчёты»: 📊 Мин. остатки | ✏️ Изменить мин. остаток | заглушка + ◀️ Назад
...
│   ├── min_stock_handlers.py # Редактирование минимальных остатков в iiko
│   │                         #   EditMinStockStates: search_product → choose_product → choose_store → enter_min_level
│   │                         #   Поиск товара → выбор → склады ресторана с текущими min → ввод нового значения
│   │                         #   Обновление через iiko REST API + локальный raw_json в БД
...
│   │                         #   v2: суммирование остатков по всем складам dept, MAX дедупликация
│   │                         #   asyncio.gather(stores, departments) — параллельные запросы
│   │                         #   format_min_stock_report(data) → str (Telegram Markdown)
│   ├── edit_min_stock.py    # Редактирование мин. остатков в iiko
│   │                         #   search_products_for_edit(query) — GOODS/PREPARED
│   │                         #   get_product_stores_levels(product_id, department_id)
│   │                         #     — склады dept + текущие min/max из raw_json
│   │                         #   update_min_level(product_id, store_id, new_min)
│   │                         #     — iiko API update + raw_json в БД
...
### 2026-02-10 — Редактирование минимальных остатков через бота

**Задача:** Добавить возможность менять минимальный остаток товара в iiko через Telegram-бот. Изменения применяются к подразделению (ресторану), в котором авторизован пользователь.

**Флоу пользователя:**
1. Меню «📊 Отчёты» → «✏️ Изменить мин. остаток»
2. Ввод названия товара → поиск (GOODS/PREPARED)
3. Выбор товара из inline-кнопок
4. Показ складов ресторана с текущими min/max уровнями
5. Выбор склада → ввод нового мин. остатка (число, 0 = убрать)
6. Обновление в iiko через REST API → обновление raw_json в БД

**Новые файлы:**
- `use_cases/edit_min_stock.py` — бизнес-логика: поиск товаров, чтение storeBalanceLevels из raw_json, обновление через API + БД
- `bot/min_stock_handlers.py` — FSM-хэндлеры: EditMinStockStates (search → choose_product → choose_store → enter_min_level), guard-хэндлеры

**Изменённые файлы:**
- `adapters/iiko_api.py` — новая функция `update_product(product_id, fields)`: POST /resto/api/v2/entities/products/update
- `bot/handlers.py` → `_reports_keyboard()` — добавлена кнопка «✏️ Изменить мин. остаток»
- `main.py` — подключён `min_stock_router` в Dispatcher

**Паттерн обновления storeBalanceLevels:**
1. Читаем raw_json продукта из БД (полный массив storeBalanceLevels)
2. Находим запись для выбранного store_id (или создаём новую)
3. Обновляем minBalanceLevel
4. Отправляем ВЕСЬ массив storeBalanceLevels в iiko (API заменяет целиком)
5. Обновляем raw_json в локальной БД (без полной пересинхронизации)

---

### 2026-02-09 — Комплексная оптимизация производительности

**Задача:** Максимальная скорость работы бота — устранение лишних round-trip к Railway PostgreSQL (~400мс каждый) и параллелизация независимых I/O-операций.

**Оптимизации:**

| Файл | Оптимизация | Экономия |
|------|-------------|----------|
| `user_context.py` | 3 последовательных SELECT → 1 JOIN (outerjoin Employee+Department+EmployeeRole) | −800мс |
| `handlers.py` → `btn_check_min_stock` | Последовательная синхронизация → `asyncio.gather(sync_products, sync_stock_balances)` | −3-5 сек |
| `check_min_stock.py` | Последовательные stores+departments → `asyncio.gather` | −400мс |
| `sync_stock_balances.py` | API fetch → DB name_maps последовательно → `asyncio.gather(API, DB)` параллельно | −300-500мс |

**Что изменено:**
- `use_cases/user_context.py` → `_load_from_db()` — outerjoin + aliased вместо 3 отдельных запросов
- `bot/handlers.py` → `btn_check_min_stock()` — asyncio.gather для двух sync-операций
- `use_cases/check_min_stock.py` → `check_min_stock_levels()` — asyncio.gather для stores+departments
- `use_cases/sync_stock_balances.py` → `sync_stock_balances()` — параллельный API-запрос и загрузка справочников из БД

**Добавлена секция «⚡ Оптимизации производительности»** в PROJECT_MAP.md с таблицей всех оптимизаций.
