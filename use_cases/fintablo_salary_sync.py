...
async def sync_fot_to_fintablo(
    ...
    fot_by_uuid, fot_by_name = fot_result
    ...
    # 2. Строим mapping → fintab + fot_data (1:1 маппинг, с fallback по имени)
    #    Маппинг может содержать iiko_id (UUID) или только fot_name.
    #    FOT может быть проиндексирован по UUID или по имени.
    stats = {
        "total": 0,
        "updated": 0,
        "skipped": 0,
    ...
    for row in mapping_rows:
        ...
        fot_name = row.get("fot_name", "")

        # Ищем FOT данные: сначала по UUID, потом по имени
        emp_data = None
        if iiko_id:
            emp_data = fot_by_uuid.get(iiko_id)
        if emp_data is None and fot_name:
            emp_data = fot_by_name.get(fot_name)
