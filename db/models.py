...
#   ocr_item_mapping   — OCR: маппинг raw_name → product_id
# ─────────────────────────────────────────────────────


# ═══════════════════════════════════════════════════════
# OCR: распознанные документы
# ═══════════════════════════════════════════════════════

class OcrDocument(Base):
    """Результат OCR-распознавания бухгалтерского документа."""
    __tablename__ = "ocr_document"

    id = Column(Integer, primary_key=True, autoincrement=True)
    telegram_id = Column(BigInteger, nullable=False, index=True)
    doc_type = Column(String(50), nullable=False, default="Неизвестный")
    doc_number = Column(String(100))
    doc_date = Column(String(20))  # "ДД.ММ.ГГГГ" как распознал LLM
    ...
    raw_json = Column(JSONB, comment="Полный JSON от LLM")
    mapped_json = Column(JSONB, comment="JSON после маппинга (с product_id, supplier_id)")
    iiko_response = Column(Text, comment="Ответ iiko при отправке")
    created_at = Column(DateTime, default=_utcnow)
    updated_at = Column(DateTime, default=_utcnow, onupdate=_utcnow)


# ═══════════════════════════════════════════════════════
# OCR: маппинг raw_name → product_id
# ═══════════════════════════════════════════════════════

class OcrItemMapping(Base):
    """
    Обучаемый маппинг: raw_name (как распознал LLM) → product_id в iiko.

    Много вариантов raw_name → один corrected_name → один product_id.
    Используется для fuzzy match при следующих распознаваниях.
    """
    __tablename__ = "ocr_item_mapping"

    id = Column(Integer, primary_key=True, autoincrement=True)
    raw_name = Column(String(500), nullable=False, index=True,
                      comment="Как распознал LLM (точно как на фото)")
    corrected_name = Column(String(500),
                            comment="Нормализованное название")
    product_id = Column(UUID(as_uuid=True),
                        comment="UUID товара в iiko (nullable если ещё не замаплен)")
    product_name = Column(String(500),
                          comment="Название товара из iiko")
    created_at = Column(DateTime, default=_utcnow)

    __table_args__ = (
        UniqueConstraint("raw_name", name="uq_ocr_item_mapping_raw_name"),
    )


class OcrSupplierMapping(Base):
    """
    Маппинг поставщиков: raw_name (как распознал LLM) → supplier_id в iiko.
    """
    __tablename__ = "ocr_supplier_mapping"

    id = Column(Integer, primary_key=True, autoincrement=True)
    raw_name = Column(String(500), nullable=False, index=True)
    raw_inn = Column(String(20))
    ...
    __table_args__ = (
        UniqueConstraint("raw_name", name="uq_ocr_supplier_mapping_raw_name"),
    )
