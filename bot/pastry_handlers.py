...
async def pastry_groups_menu(message: Message, state: FSMContext) -> None:
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="➕ Добавить группу", callback_data="pastry_add")],
        [InlineKeyboardButton(text="➖ Удалить группу", callback_data="pastry_remove_list")]
    ])
...
async def pastry_search_group(message: Message, state: FSMContext) -> None:
    ...
    for r in rows:
        buttons.append([InlineKeyboardButton(
            text=r.name,
            callback_data=f"pastry_sel:{r.id}"
        )])
    buttons.append([InlineKeyboardButton(text="❌ Отмена", callback_data="pastry_cancel")])
...
async def pastry_remove_list(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    for g in groups:
        buttons.append([InlineKeyboardButton(
            text=f"❌ {g['group_name']}",
            callback_data=f"pastry_rm:{g['id']}"
        )])
    buttons.append([InlineKeyboardButton(text="◀️ Назад", callback_data="pastry_cancel")])
