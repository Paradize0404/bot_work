...
4. Отправителю — только подтверждение «✅ Файл отправлен бухгалтеру»
5. Бухгалтеру (📑) — превью + кнопки «📤 Отправить в iiko» / «❌ Отменить»
6. Если есть незамапленные → бухгалтеру уведомление о маппинге
...
async def cb_iiko_invoice_send(callback: CallbackQuery) -> None:
    ...
    # ── Только бухгалтер может нажать ──
    from use_cases.permissions import get_accountant_ids

    accountant_ids = await get_accountant_ids()
    if tg_id not in accountant_ids:
...
async def handle_json_receipt(message: Message, state: FSMContext) -> None:
    ...
    Отправителю — только подтверждение. Всё остальное — бухгалтеру.
    """
    tg_id = message.from_user.id
    ...
    if not fully_mapped:
        # ── Незамапленные: записать в трансфер + уведомить бухгалтера ──
        await mapping_uc.write_transfer(unmapped_sup, unmapped_prd)
        ...
        await _repush(
            placeholder,
            "✅ Файл отправлен бухгалтеру на проверку.",
        )
        return
...
async def _build_and_send_json_invoices(
    ...
    await _repush(
        placeholder,
        f"✅ Файл отправлен бухгалтеру на проверку "
        f"({len(invoices)} накладных, "
        f"{sum(len(inv['items']) for inv in invoices)} позиций).",
    )

    # ── Превью + кнопки ТОЛЬКО бухгалтерам ──
    from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
